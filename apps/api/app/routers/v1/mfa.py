...
class MFADisableRequest(BaseModel):
    """MFA disable request"""
    password: str  # Require password for security
    code: Optional[str] = Field(None, min_length=6, max_length=6)  # Current TOTP code or backup code
...
async def get_mfa_status(
    ...
    if current_user.mfa_backup_codes:
        ...
        backup_codes_remaining = sum(
            1 for code in current_user.mfa_backup_codes 
...
async def get_recovery_options(
    ...
    if user.mfa_backup_codes:
        ...
        unused_codes = sum(
            1 for code in user.mfa_backup_codes
            if isinstance(code, dict) and not code.get('used', False)
        ) if isinstance(user.mfa_backup_codes, list) else len(user.mfa_backup_codes)
...
async def initiate_mfa_recovery(
    ...
    # This would include a secure link to disable MFA after email verification
