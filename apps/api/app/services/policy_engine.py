...
from app.models.policy import (
    Policy, PolicyEvaluation, Role, UserRole, RolePolicy,
    PolicyEffect, PolicyEvaluateRequest, PolicyEvaluateResponse
)
...
class PolicyEngine:
    ...
    async def evaluate(
        ...
        for policy in sorted(policies, key=lambda p: p.priority, reverse=True):
            ...
            result, reason = await self._evaluate_single_policy(
                policy=policy,
                request=request
            )
    ...
    async def _get_applicable_policies(
        ...
        user_policies = self.db.query(Policy).filter(
            and_(
        ...
        role_policies = []
        if role_ids:
            role_policy_mappings = self.db.query(RolePolicy).filter(
                RolePolicy.role_id.in_(role_ids)
            ).all()
            policy_ids = [rp.policy_id for rp in role_policy_mappings]
            
            if policy_ids:
                role_policies = self.db.query(Policy).filter(
                    and_(
        ...
        org_policies = self.db.query(Policy).filter(
            and_(
                Policy.tenant_id == tenant_id,
        ...
        resource_policies = self.db.query(Policy).filter(
            and_(
                Policy.tenant_id == tenant_id,
        ...
        for policy in resource_policies:
            if policy.resource_pattern and self._matches_pattern(
                resource, policy.resource_pattern
            ):
    ...
    async def _evaluate_single_policy(
        ...
        if policy.rules:
            rule_result = await self._evaluate_rules(
                policy.rules,
                request
            )
            if not rule_result:
    ...
    async def compile_to_wasm(self, policy: Policy) -> Optional[str]:
        ...
        This is a placeholder - in production, use OPA's WASM compilation.
        """
        # TODO: Implement OPA WASM compilation
