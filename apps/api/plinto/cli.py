"""
Plinto CLI - Command Line Interface

Provides command-line tools for Plinto setup, management, and deployment.
"""
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="plinto",
        description="Plinto - Enterprise authentication platform",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  plinto server --host 0.0.0.0 --port 8000    # Start development server
  plinto migrate                               # Run database migrations
  plinto create-user --email admin@example.com # Create admin user
  plinto version                               # Show version info
        """,
    )
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"Plinto {__version__}"
    )

    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Server command
    server_parser = subparsers.add_parser("server", help="Start the Plinto server")
    ...
    # Initialize command
    init_parser = subparsers.add_parser("init", help="Initialize Plinto configuration")
...
def init_config(config_dir: str, database_url: Optional[str] = None, redis_url: Optional[str] = None):
    ...
    env_content = f"""# Plinto Configuration
# Generated by plinto init
...
# Database
DATABASE_URL={database_url or "postgresql://postgres:postgres@localhost:5432/plinto"}
