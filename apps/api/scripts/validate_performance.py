...
class PerformanceValidator:
    ...
    async def validate_basic_endpoints(self) -> Dict[str, Any]:
        ...
        cmd = [
            "python3", "scripts/load_testing_framework.py",
            "--url", self.api_url,
            "--users", "10",
            "--duration", "30",
            "--target-ms", "50",
                    ...
                    async with session.get(url) as response:
                        ...
                        async with session.get(url) as cached_response:
                            ...
                            cache_test_results.append({
                                ...
                                'cache_improvement': first_request_time - second_request_time > 5  # >5ms improvement indicates caching
                            })
                
                # Analyze cache performance
                cache_effective = sum(1 for r in cache_test_results if r['cache_improvement']) >= len(cache_test_results) * 0.5
