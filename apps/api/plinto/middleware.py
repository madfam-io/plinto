"""
Plinto Middleware Stack

Provides a pre-configured middleware stack for FastAPI applications
integrating Plinto authentication and security features.
"""
...
def get_middleware_stack() -> List[tuple]:
    """
    Get the default Plinto middleware stack.
...
def apply_middleware_stack(app: FastAPI, custom_middleware: List[tuple] = None) -> FastAPI:
    """
    Apply the Plinto middleware stack to a FastAPI application.
    ...
    # Apply default Plinto middleware stack
...
class PlintoMiddlewareConfig:
    """Configuration class for Plinto middleware."""
...
def create_plinto_app(
    title: str = "Plinto Application",
    description: str = "Application powered by Plinto",
    version: str = "1.0.0",
    middleware_config: PlintoMiddlewareConfig = None,
    **fastapi_kwargs
) -> FastAPI:
    """
    Create a FastAPI application with Plinto middleware and configuration.
    ...
    Returns:
        Configured FastAPI application with Plinto middleware
    """
    app = FastAPI(
        title=title,
