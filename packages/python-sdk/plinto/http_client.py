...
class HTTPClient:
    """HTTP client for making API requests to the Plinto API."""
    ...
    def _handle_response_error(self, response: Response) -> None:
        ...
        Raises:
            Various PlintoError subclasses based on status code
        """
        try:
    ...
    def request(
        ...
        **kwargs
        ...
        Raises:
            Various PlintoError subclasses based on error type
        """
        ...
        while attempt <= self.max_retries:
            try:
                ...
                # Unexpected error
                raise PlintoError(f"Unexpected error during {method} {url}: {str(e)}")
        ...
        if last_error:
            ...
            raise PlintoError(f"Failed to complete request after {self.max_retries} retries")
