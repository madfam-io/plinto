...
async function main() {
  ...
  const userRole = await prisma.role.upsert({
    ...
    create: {
      ...
      permissions: [
        'user:read', 'user:update_own', 'team:read', 'payment:read_own'
      ],
      is_system_role: true,
      priority: 100
    }
  ...
  const teamLeadRole = await prisma.role.upsert({
    ...
    create: {
      ...
      permissions: [
        'user:read', 'user:update_own', 'team:read', 'team:update_own',
        'team:manage_members', 'payment:read_own'
      ],
      is_system_role: true,
      priority: 500
    }
  ...
  const stripeProvider = await prisma.paymentProvider.create({
    data: {
      ...
      type: 'card_processor',
      status: 'active',
      supported_countries: ['US', 'CA', 'GB', 'AU', 'DE', 'FR', 'IT', 'ES', 'NL', 'BE'],
  ...
  await prisma.apiKey.create({
    data: {
      ...
      key_prefix: 'pk_test',
      permissions: ['payment:read', 'payment:write', 'customer:read', 'customer:write'],
      status: 'active',
