...
class DistributedSessionManager:
    ...
    def __init__(
        ...
        # Configuration
        self.session_ttl = settings.SESSION_TTL if hasattr(settings, 'SESSION_TTL') else 3600  # 1 hour default
        self.max_sessions_per_user = settings.MAX_SESSIONS_PER_USER if hasattr(settings, 'MAX_SESSIONS_PER_USER') else 5
        self.session_idle_timeout = settings.SESSION_IDLE_TIMEOUT if hasattr(settings, 'SESSION_IDLE_TIMEOUT') else 1800  # 30 minutes
    ...
    async def create_session(
        ...
        if self.enable_concurrent_limit:
            ...
            if len(existing_sessions) >= self.max_sessions_per_user:
                ...
                oldest = min(existing_sessions, key=lambda x: x.get('created_at', datetime.utcnow()))
