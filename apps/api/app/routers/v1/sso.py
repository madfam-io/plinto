...
from ...models import Organization, SSOConfiguration, SSOProvider, SSOStatus, User
...
async def create_sso_configuration(
    ...
    try:
        ...
        if existing:
            raise HTTPException(
                status_code=400, detail="SSO configuration already exists for this organization"
            )
        ...
        if config.provider == SSOProvider.SAML:
            ...
            raise HTTPException(
                status_code=400, detail=f"Provider {config.provider} not yet supported"
            )
...
async def update_sso_configuration(
    ...
    try:
        result = await sso_service.update_configuration(
            organization_id, update.dict(exclude_unset=True)
        )
        if not result:
            raise HTTPException(status_code=404, detail="SSO configuration not found")
...
async def initiate_sso(request: SSOInitiateRequest, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if config["provider"] == SSOProvider.SAML.value:
            result = await sso_service.initiate_saml_sso(str(org.id), request.return_url)
        elif config["provider"] == SSOProvider.OIDC.value:
            result = await sso_service.initiate_oidc_sso(str(org.id), request.return_url)
        else:
            raise HTTPException(
...
async def saml_acs(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        session = UserSession(
            ...
            user_agent="SSO Login",
            ip_address="unknown",  # SAML doesn't provide client IP
            revoked=False,
        )
        ...
        logger.info("SSO session created", user_id=str(user.id), session_id=str(session.id))
...
async def saml_slo(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if saml_request:
            ...
            result = await sso_service.handle_saml_logout_request(saml_request, relay_state)
        elif saml_response:
            # Handle logout response from IdP
            result = await sso_service.handle_saml_logout_response(saml_response, relay_state)
        else:
