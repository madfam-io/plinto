...
async def list_organization_members(
    ...
    result_set = await db.execute(
        ...
        .where(organization_members.c.organization_id == org_uuid)
    )
...
async def accept_invitation(
    ...
    invitation_result = await db.execute(
        select(OrganizationInvitation).where(
            and_(OrganizationInvitation.token == token, OrganizationInvitation.status == "pending")
        )
    )
    ...
    member_check_result = await db.execute(
        select(organization_members).where(
            and_(
...
async def list_invitations(
    ...
    # Build query
    stmt = select(OrganizationInvitation).where(OrganizationInvitation.organization_id == org_uuid)
    ...
    for inv in invitations:
        ...
        result.append(
            OrganizationInvitationResponse(
                ...
                inviter_name=inviter.display_name or f"{inviter.first_name} {inviter.last_name}"
                if inviter
                else None,
                status=inv.status,
...
async def list_custom_roles(
    ...
    roles_result = await db.execute(
        select(OrganizationCustomRole).where(OrganizationCustomRole.organization_id == org_uuid)
    )
...
async def transfer_ownership(
    ...
    if not new_owner_member:
        raise HTTPException(
            status_code=400, detail="New owner must be a member of the organization"
        )
