    ...
    try:
        ...
        if not translation_key:
            translation_key = TranslationKey(
                key=translation.key,
                default_value=translation.value
            )
...
async def get_translations(
    ...
    try:
        query = select(Translation, TranslationKey).join(
            TranslationKey, Translation.translation_key_id == TranslationKey.id
        ).join(
            Locale, Translation.locale_id == Locale.id
        ...
        return {
            translation_key.key: translation.value
            for translation, translation_key in translations
        }
