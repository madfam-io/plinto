...
class InputValidator:
    ...
    def validate_password(password: str) -> Dict[str, Any]:
        ...
        if len(password) < ValidationRules.PASSWORD_MIN_LENGTH:
            issues.append(f"Password must be at least {ValidationRules.PASSWORD_MIN_LENGTH} characters")
        if len(password) > ValidationRules.PASSWORD_MAX_LENGTH:
            issues.append(f"Password must be at most {ValidationRules.PASSWORD_MAX_LENGTH} characters")
        ...
        if complexity_met < 3:
            issues.append("Password must contain at least 3 of: uppercase, lowercase, digit, special character")
        ...
        return {
            "valid": len(issues) == 0,
            "issues": issues,
            "strength": "strong" if complexity_met == 4 else "medium" if complexity_met >= 3 else "weak"
        }
        ...
        try:
            ...
            if request.method in ["POST", "PUT", "PATCH"]:
                ...
                if not validation_result["valid"]:
                    ...
                    return JSONResponse(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        content={"detail": "Invalid request body", "errors": validation_result["errors"]}
                    )
            ...
            if not validation_result["valid"]:
                ...
                return JSONResponse(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    content={"detail": "Invalid query parameters", "errors": validation_result["errors"]}
                )
            ...
            if not validation_result["valid"]:
                ...
                return JSONResponse(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    content={"detail": "Invalid path parameters", "errors": validation_result["errors"]}
                )
            ...
            if body:
                ...
                if "application/json" in content_type:
                    try:
                        ...
                        if threats:
                            errors.append(f"Potential security threats detected: {', '.join(threats)}")
    ...
    def is_validation_abuse(self, client_ip: str) -> bool:
        ...
        if client_ip in self.validation_error_tracker:
            self.validation_error_tracker[client_ip] = [
                ts for ts in self.validation_error_tracker[client_ip]
                if (now - ts).seconds < 60
            ]
        
        # Check rate
        if client_ip in self.validation_error_tracker:
            return len(self.validation_error_tracker[client_ip]) >= ValidationRules.MAX_VALIDATION_ERRORS_PER_MINUTE
