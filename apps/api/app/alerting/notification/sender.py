"""
Notification Sender
Handles sending notifications through various channels (email, Slack, webhook, Discord)
"""
...
class NotificationSender:
    ...
    async def send_notification(self, channel: NotificationChannel, alert: Alert) -> bool:
        """Send notification through the appropriate channel"""
        if not channel.enabled:
            logger.debug("Channel disabled, skipping notification", channel_id=channel.channel_id)
            return False

        try:
            if channel.channel_type.value == "email":
                return await self.send_email(channel, alert)
            elif channel.channel_type.value == "slack":
                return await self.send_slack(channel, alert)
            elif channel.channel_type.value == "webhook":
                return await self.send_webhook(channel, alert)
            elif channel.channel_type.value == "discord":
                return await self.send_discord(channel, alert)
            else:
                logger.error("Unsupported channel type",
