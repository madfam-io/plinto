...
class LoadTestingInfrastructure:
    ...
    async def _run_breakpoint_test(
        self,
        test_id: str,
        config: TestConfiguration
    ) -> TestResult:
    ...
    def _analyze_scalability(self, results: List[TestResult]) -> TestResult:
        ...
        efficiencies = [
            throughputs[i] / (base_throughput * instances[i])
    ...
    async def _generate_report(self, result: TestResult):
        ...
        if result.response_times:
            fig.add_trace(
                go.Histogram(x=result.response_times, name="Response Times"),
                row=1, col=1
            )
        ...
        fig.add_trace(
            go.Scatter(x=time_points, y=[result.requests_per_second] * len(time_points), name="RPS"),
            row=1, col=2
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=time_points,
                y=[result.peak_concurrent_users] * len(time_points),
                name="Users"
            ),
            row=2, col=2
        )
        ...
        # Update layout
        fig.update_layout(
            title=f"Load Test Report - {result.test_id}",
            height=800,
            showlegend=True
        )
    ...
    async def compare_with_baseline(
        ...
        if "p95_response_time" in baseline:
            ...
            comparison["metrics"]["p95_response_time"] = {
                ...
                "difference": diff,
                "regression": diff > baseline["p95_response_time"] * 0.1  # 10% regression threshold
            }
