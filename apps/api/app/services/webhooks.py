...
class WebhookService:
    ...
    async def trigger_event(
        ...
        try:
            ...
            tasks = [
                self._deliver_webhook(db, endpoint, event, payload)
    ...
    async def cleanup_old_events(
        ...
        deliveries_deleted = db.query(WebhookDelivery).filter(
            WebhookDelivery.created_at < cutoff
        ).delete()
        
        # Delete old events
        events_deleted = db.query(WebhookEvent).filter(
        ...
        logger.info(
            f"Cleaned up {events_deleted} events and {deliveries_deleted} deliveries"
        )
