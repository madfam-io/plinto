...
class User:
    ...
    async def organizations(self, info: Info) -> List["Organization"]:
        db = info.context["db"]
        orgs = db.query(OrgModel).filter(
            OrgModel.members.any(user_id=self.id)
        ).all()
...
class Organization:
    ...
    async def members(self, info: Info) -> List[User]:
        ...
        members = db.query(UserModel).filter(
            UserModel.organizations.any(id=self.id)
        ).all()
    ...
    async def invitations(self, info: Info, status: Optional[str] = None) -> List["Invitation"]:
        db = info.context["db"]
        query = db.query(InvitationModel).filter(
            InvitationModel.organization_id == self.id
        )
        if status:
...
class Policy:
    ...
    async def evaluations_count(self, info: Info) -> int:
        ...
        return db.query(PolicyEvaluation).filter(
            PolicyEvaluation.policy_id == self.id
        ).count()
...
class Mutation:
    ...
    async def evaluate_policy(
        self, info: Info, input: EvaluatePolicyInput
    ) -> PolicyEvaluation:
