# Janua Project Structure

## Overview
Janua is a secure identity platform with edge-fast verification, currently in private alpha. The project is structured as a multi-app system with shared design components.

## Project Layout

```
janua/
├── marketing/          # Marketing site (janua.dev)
├── dashboard/          # User dashboard (app.janua.dev)
├── admin/             # Admin portal (admin.janua.dev)
├── api/               # FastAPI backend (api.janua.dev)
...
- **Stack**: Next.js 14, Tailwind CSS, @janua/ui
- **Port**: 3001
...
Then access via:
- http://janua.local
- http://app.janua.local
- http://admin.janua.local
- http://api.janua.local
...
- `janua.dev` → Marketing
- `app.janua.dev` → Dashboard
- `admin.janua.dev` → Admin
