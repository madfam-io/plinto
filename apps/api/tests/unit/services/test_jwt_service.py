...
class TestTokenCreation:
    ...
    async def test_create_access_token(self):
        ...
        with patch('jose.jwt.encode', return_value=expected_token) as mock_encode, \
            ...
            token = await service.create_access_token(claims)
...
class TestTokenValidation:
    ...
    async def test_verify_access_token_valid(self):
        ...
        with patch('jose.jwt.decode', return_value=expected_claims) as mock_decode, \
             patch('app.models.token.TokenClaims') as mock_token_claims:
            ...
            claims = await service.verify_access_token(token)
...
class TestJWKS:
    ...
    @pytest.mark.asyncio
    async def test_get_jwks_no_keys(self):
