...
class APMConfig:
    ...
    def __init__(self):
        ...
        self.newrelic_enabled = NEWRELIC_AVAILABLE and self._get_env_bool('NEW_RELIC_ENABLED', False)
        ...
        self.opentelemetry_enabled = OPENTELEMETRY_AVAILABLE and self._get_env_bool('OTEL_ENABLED', False)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            self._record_success_metrics(
                request, response, start_time, trace_id
            )
    ...
    def _record_auth_metrics(self, request: Request, response: Response):
        ...
        if request.url.path == "/api/v1/auth/signin":
            ...
            mfa_verifications.labels(
                method="totp",  # Could be extracted from request
                status=status
            ).inc()
            ...
            async def wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    if duration > 0.5:  # Log slow queries
                        logger.warning(
                            f"Slow query detected: {query_type} on {table} "
...
class CustomMetrics:
    ...
    def track_business_event(event_name: str, properties: Dict[str, Any]):
        ...
        if DATADOG_AVAILABLE:
            from datadog import statsd
            statsd.increment(f'business.{event_name}', tags=[
